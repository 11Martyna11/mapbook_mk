...
# list to anotacja typu danych, sugestia
users: list = []
...
def update_user(i):
    ...
    users[i].marker = map_widget.set_marker(users[i].coordinates[0], users[i].coordinates[1],
                                            text=f'{user[i].name} {users[i].surname}')
...
label_miejscowosc_.grid(row=3, column=0, sticky=W)
...
button_dodaj_obiekt.grid(row=5, columnspan=2)
...
label_pokaz_szczegoly_obiektu_surname_wartosc = Label(ramka_szczegoly_obiektow, text='....')
label_pokaz_szczegoly_obiektu_surname_wartosc.grid(row=1, column=3)

label_szczegoly_obiektow_miejscowosc = Label(ramka_szczegoly_obiektow, text='Miejscowosć')
